...
ca_type try_N(N_type N, d_type d, t_type t, k_type k, v_type v, lambda_type l, const std::vector<std::tuple<d_set_type, d_set_type, int>>& non_locating_pairs) {
    ...
    int max_possible_fitness = non_locating_pairs.size();
...
ca_type go(d_type d, t_type t, k_type k, v_type v, lambda_type l, const std::vector<std::tuple<d_set_type, d_set_type, int>>& non_locating_pairs) {
    ...
    //howManyPairs = static_cast<std::size_t>(static_cast<int>(0.8*non_locating_pairs.size()));
    auto some_non_locating_pairs = non_locating_pairs;

    /*
    // Create a sub set of the non_locating_pairs to pass into the genetic algorithm
    if (non_locating_pairs.size() > 4) {
        some_non_locating_pairs.clear();
        for (v_type i = 0; i < non_locating_pairs.size()-2; i++){
            some_non_locating_pairs.push_back(non_locating_pairs[i]);
        }
    ...
    */
    ...
    while (true) {
        ...
        result = try_N(N, d, t, k, v, l, some_non_locating_pairs);
    ...
    while (N_lo < N_hi) {
        ...
        auto result2 = try_N(N_mid, d, t, k, v, l, some_non_locating_pairs);
        if (result2.size() > 0) {
            N_hi = N_mid;
            result = result2;
        } else {
...
/* KANG IMPLENTED MULTI-STAGE GENETIC ALGORITHM: also edited max fitness in try_N */
                        ...
                        while(!new_non_locating_pairs.empty()){
                            std::cout << "Repeated " << ga_rows.size() << "\n";
                            // CALL GO AND START GENETIC ALGORITHM!!
                            auto ga_rows = go(d,t,k,v,lambda,non_locating_pairs);
                            //auto requirement = lambda - num_times_sep_already;
                            for (const auto& [dset_1, dset_2, num_required] : non_locating_pairs) {
